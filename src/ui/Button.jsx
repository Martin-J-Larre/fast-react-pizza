...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    secondary:
      'inline-block rounded-full border-2 border-stone-300 font-semibold uppercase tracking-wide  hover:text-stone-800 text-stone-400 transition-colors duration-300 hover:bg-stone-300 focus:bg-stone-300 focus:text-stone-800 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed px-4 py-2.5 sm:px-6 sm:py-3.5 text-sm',
    small:
      base + ' px-4 py-2 md:px-5 md:py-2.5 text-xs px-4 py-3 sm:px-6 sm:py-4',
  };
  if (to)
    return (
